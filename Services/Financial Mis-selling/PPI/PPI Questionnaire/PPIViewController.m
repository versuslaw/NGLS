...
@interface PPIViewController ()

@end

@implementation PPIViewController
...
- (void)viewDidLoad {
    ...
    // Set navbar title
    self.navigationItem.title = @"Payment Protection Insurance";
    ...
    self.ppiDetails.delegate = self;
    [self.ppiDetails becomeFirstResponder];
    
    NSString *ppiDetails = [_managedObjectNGLS valueForKey:@"ppiDetails"];
    if (ppiDetails.length > 0) {
        self.ppiDetails.text = ppiDetails;
    }
}
...
- (IBAction)servicesBtnPressed:(UIButton *)sender {
    [self.managedObjectNGLS setValue:self.ppiDetails.text forKey:@"ppiDetails"];
