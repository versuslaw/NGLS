...
- (void)viewDidLoad {
    ...
    // Set navbar title
    self.navigationItem.title = @"Conveyancing";
    ...
    self.convDetails.delegate = self;
    [self.convDetails becomeFirstResponder];
    
    NSString *convDetails = [_managedObjectNGLS valueForKey:@"convDetails"];
    if (convDetails.length > 0) {
        self.convDetails.text = convDetails;
    }
}
...
- (IBAction)servicesBtnPressed:(UIButton *)sender {
    [self.managedObjectNGLS setValue:self.convDetails.text forKey:@"convDetails"];
